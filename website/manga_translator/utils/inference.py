...
class ModelWrapper(ABC):
    ...
    def _verify_file(self, sha256_pre_calculated: str, path: str):
    ...
    def _download(self):
        ...
        for map_key, mapping in self._MODEL_MAPPING.items():
            ...
            if 'hash' in mapping:
                ...
                if not downloaded:
                    self._download_file(mapping['url'], download_path)
                    self._verify_file(mapping['hash'], download_path)
            else:
                self._download_file(mapping['url'], download_path)
    ...
    def reload(self, device: str, *args, **kwargs):
        self.unload()
        self.load(*args, **kwargs, device=device)

    def load(self, device: str, *args, **kwargs):
        '''
        ...
        if not self.is_loaded():
            self._load(*args, **kwargs, device=device)
            self._loaded = True
    ...
    @abstractmethod
    def _load(self, device: str, *args, **kwargs):
        pass
