...
class CommonDetector(InfererModule):
    def _merge_textlines(self, textlines: List[Quadrilateral], img_width: int, img_height: int, verbose: bool = False) -> List[TextBlock]:
        return dispatch_textline_merge(textlines, img_width, img_height, verbose)

    def detect(self, image: np.ndarray, detect_size: int, text_threshold: float, box_threshold: float,
        ...
        text_regions, raw_mask, mask = self._detect(image, detect_size, text_threshold, box_threshold, unclip_ratio, det_rearrange_max_batches, verbose)
        ...
        if auto_orient:
            ...
            if majority_orientation == 'h':
                ...
                text_regions, raw_mask, mask = self.detect(rot_image, detect_size, text_threshold, box_threshold, unclip_ratio, det_rearrange_max_batches, auto_orient=False)
    ...
    @abstractmethod
    def _detect(self, image: np.ndarray, detect_size: int, text_threshold: float, box_threshold: float,
