...
class ComicTextDetector(OfflineDetector):
    ...
    def _load(self, device: str, input_size=1024, half=False, nms_thresh=0.35, conf_thresh=0.4):
        self.device = device
        if self.device == 'cuda':
    ...
    def _infer(self, image: np.ndarray, detect_size: int, text_threshold: float, box_threshold: float,
        ...
        text_regions = self._merge_textlines(textlines, image.shape[1], image.shape[0], verbose=verbose)
